...
import { ONBOARD } from '../../client/mutations'
import { withOnboardingRedirector } from '../../components/onboarding-redirector'
...
const Onboarding = () => {
	...
	const [submitError, changeSubmitError] = useState(null)
	...
	const submitData = () => {
		const payload = {
			name: state.name,
			mentorMentee: state.mentorMentee,
			fieldStudy: state.fieldStudy,
			intendedMajor: state.intendedMajor,
			gradYear: parseInt(state.gradYear),
			race: state.race,
			gender: state.gender,
			finaid: state.finaid,
			applyIvy: state.schoolTypes.ivy,
			applyStateFlagships: state.schoolTypes.stateFlagships,
			applyOtherState: state.schoolTypes.otherState,
			applyOtherPrivate: state.schoolTypes.otherPrivate,
		}
		changeSubmitError(null)
		onboard({ variables: { profile: payload } })
			.then(() => router.push('/forum'))
			.catch(err => changeSubmitError(
				<div className="py-8 px-4">{JSON.stringify(err)}</div>,
			))
	}
...
export default withOnboardingRedirector(Onboarding, true)
