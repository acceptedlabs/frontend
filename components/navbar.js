	...
	<div className="w-screen px-6 md:px-16 lg:px-16 xl:px-16 py-8 grid sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 grid-flow-col">
		...
		{!rightPanelHidden ? <div className="col-span-1 sm:col-start-2 md:col-start-2 lg:col-start-4 text-right flex items-center justify-end">
			{!auth.isAuth ?
				...
				<p className="text-md"><span className="font-medium">{auth.profile.given_name}</span>&emsp;<Link href="/auth/signout"><span className="text-blue-600 hover:text-black cursor-pointer">Sign out</span></Link></p>
			}
		</div>
			: null}
	</div>
